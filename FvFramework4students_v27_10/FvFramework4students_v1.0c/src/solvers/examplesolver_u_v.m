...
%
%==========================================================================
function result_u_v = examplesolver(casedef)
...
while iterate   
   ...
   for i = 1:nIf
      ...
      %bVector
      b_u(nb1) = - dPx/dx * volPc(nb1) / rho + U_old(1,nb1) * volPc(nb1)/dt;
      b_u(nb2) = - dPx/dx * volPc(nb2) / rho + U_old(1,nb2) * volPc(nb2)/dt;
   end
 ...
 %V COMPONENT  
   ...
   for i = 1:nIf
      ...
      %bVector
      b_v(nb1) = - dPy/dy * volPc(nb1) / rho + V_old(2,nb1) * volPc(nb1)/dt;
      b_v(nb2) = - dPy/dy * volPc(nb2) / rho + V_old(2,nb2) * volPc(nb2)/dt;
   end
   % Compute coefficients for ghost cell eqns and add them to eqn object
   for i = 1:nBf
      nb1 = fNbC(2*i-1 + 2*nIf); % Physicl cell
        ...
        if ismember(i + nIf,ranges(1,j):ranges(2,j)) && boundaryFound == 0
            ...
            switch BC{j}.kind
                case 'Dirichlet' 
                    ...
                    % Off-diagonal elements: ghost cells
                    aNb_bound_v(2*i-1) = aNb_bound_v(2*i-1) + k*Af/Xi + outw*Af*V_n*XiLambda;
...
result_u_v.Tconverged_u_v = u_v_converged;
result_u_v.niter = niter;
result_u_v.resnorm_u = resnorm_u;
result_u_v.resnorm_v = resnorm_v;
result_u_v.res_u = Field(dom.allCells,0);
   set(result_u_v.res_u,res_u');
result_u_v.res_v = Field(dom.allCells,0);
   set(result_u_v.res_v,res_v');
