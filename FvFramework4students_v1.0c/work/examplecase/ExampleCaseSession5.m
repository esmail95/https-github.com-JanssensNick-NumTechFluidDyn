...
switch MMS_case
   case 'example'
      ...
      %Source = @(x,y) [2*x.^3 - 2*k ; -2*y.^3 + 2*k]; % Analytically computed source term
      %Source = @(x,y) [2*x.^3 - 2*k ; 2*y.^3 + 2*k]; % Analytically
      casedef.vars.Source = Source;
   case 'degree1'
      U_chosen = @(x,y) [x;y];  % Imposed pressure field
      set(U_sol,U_chosen(casedef.dom.cCoord(1,:),casedef.dom.cCoord(2,:)));
      set(U_diff,U_chosen(casedef.dom.cCoord(1,:),casedef.dom.cCoord(2,:))); 
      % Derivation in slides is wrong, k must be included
      Source = @(x,y) [x;y]; % Analytically computed source term
      casedef.vars.Source = Source;
end
